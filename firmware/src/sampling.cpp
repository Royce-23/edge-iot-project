...
bool collectWindowWithDiagnostics(float* ax, float* ay, float* az,
    ...
    auto finishMetrics = [&]() {
        uint32_t timerEnd = 0, bufferEnd = 0, errorsEnd = 0;
        readCounters(timerEnd, bufferEnd, errorsEnd);
        if (collected >= 2) {
            metrics = calculateMetrics(gWindowTimestamps, collected,
                timerEnd - timerStart, bufferEnd - bufferStart, errorsEnd - errorsStart);
        } else {
            metrics.targetSampleRateHz = static_cast<float>(P2_SAMPLE_RATE_HZ);
            ...
            metrics.droppedSamples = metrics.timerOverruns + metrics.bufferOverruns +
    ...
    finishMetrics();
    // A window with known missing/error samples is not consecutive and must
    // never reach P3 classification, even when its average rate looks correct.
    return std::isfinite(metrics.actualSampleRateHz) &&
           metrics.actualSampleRateHz > 0.0f && metrics.droppedSamples == 0;
}
