...
bool configureClassifier(const ClassifierConfig& config) {
    ...
    if (!std::isfinite(config.rmsThreshold) || config.rmsThreshold < 0.0f ||
        ...
        !std::isfinite(config.scoreClearThreshold) ||
        config.scoreClearThreshold < 0.0f ||
        config.scoreClearThreshold >= config.scoreThreshold) {
        return false;
    }
...
bool getClassifierScoreThresholds(float& enterThreshold,
    ...
    enterThreshold = activeConfig.scoreThreshold;
    clearThreshold = activeConfig.scoreClearThreshold;
    return std::isfinite(enterThreshold) && enterThreshold > 0.0f &&
           std::isfinite(clearThreshold) && clearThreshold >= 0.0f &&
           clearThreshold < enterThreshold;
}
