...
struct ClassifierConfig {
    ...
    float scales[4];
    float weights[4];
    float scoreThreshold;       // Enter WARNING at or above this score.
    float scoreClearThreshold;  // Leave WARNING only below this score.
};
...
// Experimental fan band selected from the first real normal-running captures
// (about 208-230 Hz at 800 Hz sampling). Keep it centralized so firmware and
// host tests cannot silently use different bands. P3 must revalidate this band
// after collecting multiple normal and abnormal runs at the final fan speed.
constexpr float P3_EXPERIMENTAL_BAND_LOW_HZ = 200.0f;
constexpr float P3_EXPERIMENTAL_BAND_HIGH_HZ = 260.0f;
...
bool getClassifierScoreThresholds(float& enterThreshold,
